...
class DatabaseSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        // заполнение таблицы типы тренировок
        ...
        for ($i = 1; $i <= Utils::$count_coaches; $i++) {
            ...
            $arr_time_edit_personal_schedule[] = ['date_edit' => Utils::randomDate(Utils::decMonths(date('Y-m-d'), 9),date('Y-m-d')),'coach_id' => $i];
        }
        ...
        Schedule::factory(45)->create();

        //расписания персональных тренировок
        for ($i = 1; $i <= Utils::$count_coaches; $i++) {
            for($j = 1; $j <= 7; $j++){
                for ($q = 8; $q <= 18; $q+=2){
                    $personal_schedule[] = ['day_id' => $j,
                                            'time_begin' =>  str_pad($q, 2, "0", STR_PAD_LEFT).":00",
                                            'coach_id' => $i];
        ...
        DB::table('personal_schedules')->insert($personal_schedule);

        // заполнение таблицы групповые тренировки
        GroupWorkout::factory(4200)->create();

        $arr_sing_personal = array();
        // цикл по клиентам. Для каждого клиента создаем соответствующие записи: покупка абонементов, регистрация на
        ...
        for ($i = 1; $i <= Utils::$count_customers; $i++) {
            ...
            if($i % 2 != 0) {
                ...
                Utils::singUpPersonalWorkout($arr_sing_personal, $faker, $date, $i, $limited_price_list_id);
